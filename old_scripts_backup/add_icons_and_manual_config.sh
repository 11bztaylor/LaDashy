#!/bin/bash
# Add service icons and manual configuration ability

cd /home/zach/homelab-documentation/ladashy_unified

# First, let's add the service icons to the frontend
cat >> frontend/service-icons.js << 'EOFICONS'
// Service icon mappings - using emoji for now, can be replaced with actual images
const SERVICE_ICONS = {
    // Media Services
    'plex': '🎬',
    'jellyfin': '🎭',
...
function getServiceIcon(serviceName) {
    ...
    return SERVICE_ICONS[normalized] || SERVICE_ICONS['default'];
}
EOFICONS

# Now update the HTML to include manual configuration
cat > frontend/index_updated.html << 'EOFHTML'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
            ...
            <div class="modal-content">
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="manual-port" placeholder="Leave empty for default">
                    </div>
    ...
    <script>
        ...
        function selectServiceType(service) {
            ...
            // Set default port
            document.getElementById('manual-port').value = service.defaultPort;
        }
        ...
        async function addManualService() {
            ...
            const port = document.getElementById('manual-port').value || selectedServiceType.defaultPort;
...
# Replace the old index.html with the updated one
mv frontend/index_updated.html frontend/index.html

echo "✅ Added service icons and manual configuration!"
